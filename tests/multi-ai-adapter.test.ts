...
describe('Agent 2 & 3 : Multi-Model AI Engine (Anthropic, Gemini, Unsplash)', () => {
  ...
  it('generates English variants and uses English label names when language is en', async () => {
    let sentBody: any = null;
    const mockOpenAiFetch = vi.fn().mockImplementation(async (url: string, opts: any) => {
            ...
            choices: [
              {
                message: {
                  content: JSON.stringify({
                    variants: [
                      { label: 'concise', content: 'Concise English post generated by OpenAI.', suggestedHook: 'High impact hook' },
                      { label: 'expert', content: 'Expert English post.' },
                      { label: 'executive', content: 'Executive English post.' },
                      { label: 'pedagogical', content: 'Educational English post.' },
                      { label: 'high-engagement', content: 'High-engagement English post.' },
                    ],
                  }),
                },
              },
    ...
    const adapter = new NativeAiAdapter();
    const variants = await adapter.generatePostVariants({
      dossierTitle: 'Global Semiconductor Supply Chains',
      dossierSummary: 'Strategic analysis of export controls and foundry expansions.',
      network: 'LINKEDIN',
      tone: 'Executive & Visionary',
      language: 'en',
    });

    expect(variants).toHaveLength(5);
    expect(variants[0]?.name).toBe('Concise Version');
    expect(variants[1]?.name).toBe('Expert Deep-Dive');
    expect(variants[2]?.name).toBe('Executive / C-Level');
    expect(variants[3]?.name).toBe('Educational Breakdown');
    expect(variants[4]?.name).toBe('High Engagement / Debate');
    expect(sentBody?.messages?.[0]?.content).toContain('CRITICAL RULES:');
    expect(sentBody?.messages?.[0]?.content).toContain('flawless, idiomatic English');
